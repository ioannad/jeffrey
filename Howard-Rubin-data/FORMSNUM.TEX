% This file has been kindly provided by Prof. Paul Howard. 
% I have changed two appearences of Lindel\"{o}f to Lindel\"of, to ease parsing in read-forms.lisp. 
% I made some small changes and addition, to keep the parsing functions reasonable.
...
\topmatter
\title\chapter\nofrills{Part I} Numerical List of Forms \endtitle
\leftheadtext{Part I: Numerical List of Forms}
\endtopmatter
\medskip
\noindent{\bf FORM 0.} $0 = 0$.
\smallskip
\item{}{\bf [0 A]}  Cardinal successors 2:  For every cardinal $m$ there is
a cardinal $n$ such that $m < n$ and $(\neg  \exists  p)(m < p < n)$.
\ac{Tarski} \cite{1954a} and \ac{Jech} \cite{1966a}.
\smallskip
\item{}{\bf [0 B]} If $X$ is finite then $X^\omega$ is compact.
\ac{Brunner} \cite{1984b}.
\smallskip
\item{}{\bf [0 C]}  Closed subspaces of limited amorphous spaces are
limited amorphous.  (A space is {\it limited amorphous} if each amorphous
subset is compact.)  \ac{Brunner} \cite{1984b}.
\smallskip
\item{}{\bf [0 D]}  Products of limited amorphous spaces are limited
amorphous. (A space is {\it limited amorphous} if each amorphous
subset is compact.)  \ac{Brunner} \cite{1984b}
\smallskip
\item{}{\bf [0 E]}  The box product of a well ordered family of limited
amorphous spaces is limited amorphous.  (A space is {\it limited amorphous}
if each amorphous subset is compact.)  \ac{Brunner} \cite{1984b} and
note 52.
\smallskip
\item{}{\bf [0 F]}  Cancellation  for surjective cardinal equivalence:
For every $k\in\omega -\{ 0\}$, $(\forall x\forall y)(kx =^* ky$
implies $x =^* y)$ (The expression $x =^* y$ means that there is a
function $f$ from $x$  onto $y$ and a function $g$ from $y$ onto $x$.)
\ac{Truss} \cite{1984}
\smallskip
\item{}{\bf [0 G]}  Cancellation for injective cardinal inequality:
For every $k\in\omega - \{ 0\}$, $(\forall x \forall y)(kx \le ky$
implies $x\le y)$ ($\le $ is the usual (injective) cardinal ordering.)
...
\cite{1908b} and \ac{Abian} \cite{1980}. \iput{Zermelo's fixed point
theorem}
\smallskip
...
\iput{Caristi's fixed point theorem}
\smallskip
...
\cite{1982}. \iput{hereditarily countable set}
\smallskip
...
$|x| < |w(x)|$ where $w(x)$ is the set of all well orderings of subsets of
...
\itemitem{(a)} $(X,\le)$ is upper semi-continuous with respect to $\tau$,
\itemitem{(b)} Every well ordered subset of $(X,\le)$ has a unique
limit as a net, \par
\item{}then every function $f:X\to X$ such that $\forall x\in X$,
$x\le f(x)$ has a fixed point. \ac{Manka} \cite{1988b} and note 38.
\iput{Turinici's fixed point theorem}
\smallskip
...
\iput{constructively complete lattice}
\smallskip
...
is separable.  \ac{Gutierres} \cite{2004} and note 159. \iput{super second
countable}
\medskip
\medskip
...
\ac{Brunner} \cite{1989} and note 22. \iput{Hajnal's free set principle}
\smallskip
...
\iput{ramification lemma}
\smallskip
\item{}{\bf [1 N]}  For every set mapping $f: X\rightarrow [X]^{WO}$
(well ordered subsets of $X$) there is a co-well-orderable free subset
...
bijection $h : X \rightarrow  Y$ such that $h \subseteq f \cup g^{-1}$.
\ac{Banaschewski/Moore} \cite{1990}. \iput{Cantor-Bernstein theorem}
\smallskip
\item{}{\bf [1 Z]}  If $(P,\le)$ is a quasi-order and $P\neq\emptyset$
and every antichain is finite, then there is a $P$ generic filter which
is well orderable.  \ac{Shannon} \cite{1990} and note 47.
\smallskip
\item{}{\bf [1 AA]}  Assume $f: S\rightarrow R$ where $f$ is surjective
and $R\subseteq S$.
Then every mapping $g$ of $S$ into $R$ has the form $g = f\circ h$ for
some $h: S\rightarrow S$.  \ac{Smith} \cite{1982}.
\smallskip
\item{}{\bf [1 AB]}  Assume $f: S\rightarrow R$ where $f$ is surjective
and $R\subseteq S$.
Then the mappings of $S$ onto $R$ are precisely the mappings of the form
$f\circ h$  where $h: S\rightarrow S$ and $(\forall y\in R)
(\exists x\in S)(h(x)\in f^{-1}(\{y\}))$.  \ac{Smith} \cite{1982}.
\smallskip
\item{}{\bf [1 AC]}  Assume $f: S\rightarrow R$ where $R\subseteq S$.
Then the retractions of $S$ onto $R$ (that is, the mappings $g: S
\rightarrow R$ which are the identity on $R)$ are precisely the
mappings $f\circ h$ where $h: S\rightarrow S$ and
$(\forall y\in R)(h(y)\in f^{-1}(\{y\}))$.  \ac{Smith} \cite{1982}.
\smallskip
\item{}{\bf [1 AD]}  For every class $K$ of similar algebras $SP^{g} K
\supseteq P^{g}S\ K$.  (For any class $K$ of similar algebras, $S\ K$ is
the class of all subalgebras of elements of $K$ (not closed under
isomorphism) and $P^{g}K$ is the class of direct products of elements of
$K$ not closed under isomorphism).  \ac{Andreka/Nemeti} \cite{1980}.
\smallskip
\item{}{\bf [1 AE]}  There is a set $S$ such that $SVC$ holds with $S$
and such that ${\Cal P}(S) - \{\emptyset\}$ has a choice function.
\ac{Blass} \cite{1979} and  note 59. Also see form 191.
\smallskip
\item{}{\bf [1 AF]}  Form 191 + form 8 + form 193. \ac{Blass} \cite{1979}
and note 60.
\smallskip
\item{}{\bf [1 AG]}  Form 192  + form 191. \ac{Blass} \cite{1979} and
note 60.
\smallskip
\item{}{\bf [1 AH]}  $E(V,VI)$: For all infinite cardinals $m$, $2m = m$
implies $m^{2} = m$.  \ac{Howard/Yorke} \cite{1989} and note 94.
\smallskip
\item{}{\bf [1 AI]}  $E(VI,VII)$:  For all infinite cardinals $m$,
$m^{2} = m$ implies there is an ordinal $\alpha$ such that $m =
...
that
\itemitem{(a)} $(X,\le)$ is upper semicontinuous with respect to $\tau$,
\itemitem{(b)} Every well ordered subset of $(X,\le)$ is convergent as a
net, \par
\item{}then every function $f:X\to X$ such that $\forall x\in X$,
$x\le f(x)$ has a fixed point. \ac{Manka} \cite{1988b} and note 38.
\iput{Turinici's fixed point theorem}
...
\iput{Birkhoff representation theorem}
\smallskip
...
subspace. \ac{Schnare} \cite{1968} and note 106.
\smallskip
\item{}{\bf [1 BJ]} In every vector space over the two element field, every
generating set contains a basis. \ac{Keremedis} \cite{1996a}.
\smallskip
\item{}{\bf [1 BK]} Every open cover $\Cal{U}$ of a metric space $(X,d)$
can be written as a well ordered union $\bigcup\{U_\alpha :
\alpha\in\gamma\}$ where $\gamma$ is an ordinal and each $U_\alpha$ is
pairwise disjoint. \ac{Howard/Keremedis/Rubin/Stanley} \cite{1999}.
\smallskip
\item{}{\bf [1 BL]} Every family of closed sets in a $T_2$ topological
space includes a maximal subfamily with the finite intersection
property.  \ac{Keremedis} \cite{1998a} and  \ac{H.~Rubin/J.~Rubin}
\cite{1985}, p 60, $M14$.
\smallskip
\item{}{\bf [1 BM]} For every family $\{(X_i,T_i): i\in k\}$ of $T_1$
topological spaces, the box topology contains an element
$\prod_{i\in k}O_i$, such that $\emptyset\ne O_i$ and $O_i\ne X_i$ for all
$i\in k$. \ac{Keremedis} \cite{1998a} and note 77.
\smallskip
\item{}{\bf [1 BN]} For every family $\{(X_i,T_i): i\in k\}$ of disjoint
 $T_1$ topological spaces, the free union contains an open set $O$
such that $\emptyset\ne O\cap X_i$ and $ O\cap X_i\ne X_i$ for all
$i\in k$. \ac{Keremedis} \cite{1998a} and note 77.
\smallskip
\item{}{\bf [1 BO]} For every $T_1$ topological space $(X,T)$ and every
base (subbase) $B$ for $X$ there exists a $\subseteq$-maximal cellular
family $Q\subseteq B$. \ac{Keremedis} \cite{1998a} and note 77.
\smallskip
\item{}{\bf [1 BP]} For every dense in itself topological space $(X, T)$
and every base $B$ for $X$ there exists a nest $N$ included in $B$.
\ac{Keremedis} \cite{1998a} and note 77.
\smallskip
\item{}{\bf [1 BQ]} For every dense in itself topological space $(X,T)$
and every base $B$ for $X$ there exists a tower $R$ included in $B$.
\ac{Keremedis} \cite{1998a} and note 77.
\smallskip
\item{}{\bf [1 BR]} For every $T_2$ space $(X,T)$ and every subbase
$\Cal B$ for $X$, if $\Cal C$ and $\Cal D$ are cellular families of $X$
included in $\Cal B$, then there is a cellular family $\Cal E\subseteq
\Cal B$ such that $\Cal C$ and $\Cal D$ are equipollent to subsets of
$\Cal E$. \ac{Keremedis} \cite{1998a} and note 77.
\smallskip
\item{}{\bf [1 BS]} For every Abelian group $(G,+)$, for every $H\subseteq
G$, the infinite system of equations
$$ x_i + y_i = a_i,\ i\in I,\ a_i\in G$$
has a solution in $H$ iff each of its equations has a solution in $H$.
\ac{Keremedis} \cite{1996b}.
\smallskip
\item{}{\bf [1 BT]} For every Boolean ring $(B,+,\cdot)$ and every $H
\subseteq  B$  which is closed under $+$ there exists a
$\subseteq$-maximal ideal $Q\subseteq B$ such that $H\cap Q=\{0\}$.
\ac{Keremedis} \cite{1996b}.
\smallskip
\item{}{\bf [1 BU]} In every vector space over ${\Bbb Q}$, every
generating  set includes a basis.  \ac{Blass} \cite{1984a} and note 137.
\smallskip
\item{}{\bf [1 BV]} If $(X,T)$ is a $T_4$ topological space and $G =
\{G_i: i\in K\}$, $|G_i|\ge 2$, is a locally finite family of
compact sets with non-empty pairwise disjoint interiors, then
there is a continuous real valued function $f$ on $X$ which
assumes at least two distinct values on each $G_i$.  \ac{Keremedis}
\cite{1997} and note 43.
\smallskip
\item{}{\bf [1 BW]} If $(X,T)$ is a $T_4$ topological space and $U =
\{U_i: i\in K\}$, $|U_i|\ge 2$, is a locally finite family of
open non-empty pairwise disjoint sets, then there is a continuous
real valued function $f$ on $X$ which assumes at least two distinct
values on each $U_i$. \ac{Keremedis} \cite{1997} and note 43.
\smallskip
\item{}{\bf [1 BX]} For every Abelian group $G$ and every subgroup $H$ of
$G$ there exists a set of representatives for the quotient $G/H$.
\ac{Keremedis} \cite{1996b}.
\smallskip
\item{}{\bf [1 BY]} If $x$ is a set of sets of cardinality at least 2,
then there exists a function $f$ such that for each $u\in x$, $f(u)$ is
a finite, non-empty, proper subset of $u$. (Form [1 BY] implies [62 E]
($KW(\infty,<\aleph_0)$) and 67 (MC) and $62 + 67\to$ AC.) \ac{Blass}
\cite{1979}.
\smallskip
\item{}{\bf [1 BZ]} Vector Space Multiple Choice:  For every family
$V = \{\,V_i : i \in K\,\}$ of non-trivial vector spaces there is
a family $F = \{\, F_i : i\in K\,\}$ such that for each $i\in K$,
$F_i$ is a non-empty, finite, independent subset of $V_i$.
Note 127.
\smallskip
\item{}{\bf [1 CA]} Strong Nielsen-Schreier Theorem I:  If $G$ is a group
which is freely generated by $X$ and $U$ is a subgroup of $G$, then there
...
note 127. % added missing \smallskip at line 784
\smallskip 
...
\iput{directed set} \iput{constructively directed set}
\smallskip
...
\medskip
\noindent{\bf FORM 2.} Existence of successor cardinals: For every cardinal
$m$ there is a cardinal $n$ such that $m < n$ and $(\forall p < n)(p \le
m)$.  \ac{Tarski} \cite{1954a}.
\medskip
\noindent{\bf FORM 3.} $2m = m$: For all infinite cardinals $m$, $2m = m$.
\ac{Tarski} \cite{1954a}.
\rightheadtext{Form 3: $2m = m$}
\smallskip
...
\medskip
\noindent{\bf FORM 4.} Every infinite set is the union of some disjoint
family of denumerable subsets. (Denumerable means $\cong \aleph_0$.)
\ac{Whitehead} \cite{1902}.
\medskip
\noindent{\bf FORM 5.} $C(\aleph_0,\aleph_0,\Bbb R)$: Every denumerable
...
\medskip
\noindent{\bf FORM 6.} $UT(\aleph_0,\aleph_0,\aleph_0,\Bbb R)$: The
...
\medskip
\noindent{\bf FORM 7.} There is no infinite decreasing sequence of cardinals.
\ac{Schoenflies} \cite{1908}.
\medskip
\noindent{\bf FORM 8.} $C(\aleph_{0},\infty)$:  Every denumerable family of
non-empty sets has a choice function.  \ac{Sierpi\'nski} \cite{1918}.
\iput{countable axiom of choice}
\smallskip
...
separable.  \ac{Gutierres} \cite{2004} and note 159. \iput{super second
countable}
\smallskip
...
note 21.
\medskip
\noindent{\bf FORM 10.}  $C(\aleph_{0},< \aleph_{0})$:  Every denumerable
...
\ac{De la Cruz/Di Prisco} \cite{1998a}, \cite{1998b}.
\medskip
\noindent{\bf FORM 13.} Every Dedekind finite subset of ${\Bbb R}$ is
...
\medskip
\noindent{\bf FORM 14.} BPI: Every Boolean algebra has a prime ideal.
\ac{M.~Stone} \cite{1936}.
\rightheadtext{Form 14: The Boolean Prime Ideal Theorem}
\smallskip
...
\ac{M.~Stone} \cite{1936}. \iput{Stone representation theorem}
\smallskip
...
\cite{1954}. \iput{Tychonoff's theorem}
\smallskip
...
\iput{Stone-\v Cech compactification theorem}
\smallskip
...
and note 21. \iput{Konig's lemma}
\smallskip
...
\ac{Rubin/Scott} \cite{1954} and note 23. \iput{Alaoglu's theorem}
\smallskip
...
Assume
\itemitem{(a)} $\Phi(\lambda) = \{f(\lambda ): f\in\Phi$ and $\lambda\in$
dom $f\}$ is finite for each $\lambda\in \Lambda$,
\itemitem{(b)} each finite $S\subseteq\Lambda$ is the domain of at
least one $f\in\Phi$,  and
\itemitem{(c)} $\Phi$ has finite character, (that is, a function $f$ from
...
\cite{1996a}. (See [14 I].) \iput{Cowen-Engler lemma}
\smallskip
\item{}{\bf [14 Y]}  Form 99  + Form 62. Note 33.
\iput{Rado's selection lemma}
\smallskip
...
\cite{1967}, and note 34. \iput{Alexander's subbase lemma}
\smallskip
...
\{0,1\}$ and such that
\itemitem{(a)} For every finite $F\subseteq S$, $\exists \phi \in {\Cal E}$
with domain F.
\itemitem{(b)} The restriction of any function in ${\Cal E}$ to any subset
of its domain is in ${\Cal E}$.
\itemitem{(c)}If $\phi $ is any function with domain $\phi\subseteq S$  such
...
\ac{Rav} \cite{1977}. \iput{Engelers lemma}
\smallskip
...
\iput{Robinson's valuation lemma}
\smallskip
...
satisfying
\itemitem{(1)} $(\forall K\in T)(\exists S\in{\Cal F})(K\in S)$
\itemitem{(2)} $(\forall S_{1}, S_{2}\in{\Cal F})(\exists S_{3}\in
...
\cite{1972a}. \iput{Monteiro's theorem}
\smallskip
...
\ac{B\'en\'ejam} \cite{1970}. See [14 Y]. \iput{Fra\"i ss\'e's theorem}
\smallskip
...
notes 23 and 29. \iput{Gelfand extreme point theorem}
\smallskip
...
\ac{Kolany} \cite{1999}. \iput{Kolany patching lemma}
\smallskip
...
conditions are equivalent:
\itemitem{(1)} $F$ is compact
\itemitem{(2)} (a) For each $x\in X$, the set $F(x) =\{f(x): f\in F\}$
is compact in $Y$.
...
\itemitem{(1)} For each $j\in V$, the variable $x_j$, has a finite domain,
$D_j$.
\itemitem{(2)} Given any finite number of variables, there is an equation
in the system (*) which contains those variable and possibly others.
\itemitem{(3)} For each equation in the system (*) there is an equation in
...
\smallskip
\item{}{\bf [14 DC]} \icopy{Clifford's Theorem}. An \icopy{abelean $l$-group}
is isomorphic to a subdirect product of totally ordered abelian groups.
\ac{Gluschankof} \cite{1995} and note 151.
\smallskip
\item{}{\bf [14 DD]} \icopy{Lorenzen's Theorem}. A representable $l$-group is
isomorphic to a subdirect product of totally ordered groups.
\ac{Gluschankof} \cite{1995} and note 151.
\smallskip
\item{}{\bf [14 DE]} \icopy{Holland's Theorem}. Any $l$-group is isomorphic
...
\medskip
\noindent{\bf FORM 15.} $KW(\infty,\infty)$ (KW), The Kinna-Wagner Selection
...
\medskip
\noindent{\bf FORM 16.}  $C(\aleph_{0},\le 2^{\aleph_{0}})$:  Every denumerable
...
\medskip
\noindent{\bf FORM 17.} Ramsey's Theorem I: If $A$ is an infinite set and the
...
11.20 and \ac{Ramsey} \cite{1929}. \iput{Ramsey's theorem}
\medskip
\noindent{\bf FORM 18.} $PUT(\aleph_{0},2,\aleph_{0})$:  The union of a
...
\medskip
\noindent{\bf FORM 19.} A real function is analytically representable if
and only if it is in \icopy{Baire's classification}. \ac{G.~Moore}
\cite{1982} (2.3.1).
\medskip
\noindent{\bf FORM 20.} If $\{A_{x}: x \in S \}$ and $\{B_{x}:
...
\medskip
\noindent{\bf FORM 21.} If $S$ is well ordered, $\{A_{x}: x\in S\}$ and
...
\medskip
\noindent{\bf FORM 22.} $UT(2^{\aleph_{0}},2^{\aleph_{0}},2^{\aleph_{0}})$:
...
\medskip
\noindent{\bf FORM 23.} $(\forall \alpha)(UT(\aleph_{\alpha},\aleph_{\alpha},
...
\medskip
\noindent{\bf FORM 29.} If $|S| = \aleph_{0}$ and $\{A_{x}: x\in S\}$ and
...
\medskip
\noindent{\bf FORM 30.} Ordering Principle: Every set can be linearly
ordered.  \ac{Schr\"oder} \cite{1988}. \iput{ordering principle}
\rightheadtext{Form 30: Ordering Principle}
\smallskip
...
$\subseteq$-chain.  Note 130. \iput{Hausdorff lemma}
\medskip
\noindent{\bf FORM 31.} $UT(\aleph_{0},\aleph_{0},\aleph_{0})$:
The countable union theorem:   The union of a denumerable set
of denumerable sets is denumerable.  \ac{Cantor} \cite{1878}.
\medskip
\noindent{\bf FORM 32.} $C(\aleph_0,\le\aleph_0)$: Every denumerable set of
...
function.
\medskip
\noindent{\bf FORM 33($n$).} If $n\in\omega-\{0,1\}$, $C(LO,n)$:  Every
linearly ordered set of $n$ element sets has  a choice function.
\ac{Truss} \cite{1973a}.
\medskip
\noindent{\bf FORM 34.} $\aleph_{1}$ is regular. \ac{Russell}
...
\medskip
\noindent{\bf FORM 35.} The union of countably many meager subsets
...
\medskip
\noindent{\bf FORM 36.}  Compact T$_2$ spaces are Loeb. (A space is
{\it Loeb} if the set of non-empty closed sets has a choice function.)
\ac{Keremedis/Tachtsis} \cite{2000}.
\medskip
\noindent{\bf FORM 37.} Lebesgue measure is countably additive.
\ac{Sierpi\'nski} \cite{1916}.
\medskip
\noindent{\bf FORM 38.} ${\Bbb R}$ is not the union of a countable family
...
$2^{\aleph_{0}}$.
\medskip
\noindent{\bf FORM 39.}  $C(\aleph_{1},\infty)$: Every set $A$ of non-empty
sets such that $\vert A\vert = \aleph_{1}$ has a choice function.
\ac{Sierpi\'nski} \cite{1918}, G\. \ac{Moore} \cite{1982} p. 202.
\medskip
\noindent{\bf FORM 40.} $C(WO,\infty)$:  Every well orderable set of
...
and note 94.
\medskip
\noindent{\bf FORM 41.} $W_{\aleph _{1}}$: For every cardinal $m$, $m \le
\aleph_{1}$ or $\aleph_{1}\le m $.  \ac{Jech} \cite{1973b}.
\medskip
\noindent{\bf FORM 42.} L\"owenheim-Skolem Theorem:  If a countable
...
\iput{L\"owenheim-Skolem theorem}
\medskip
\noindent{\bf FORM 43.} $DC(\omega)$ (DC), Principle of Dependent Choices:
...
and note 54.
\rightheadtext{Form 43: Principle of Dependent Choices}
\smallskip
...
\cite{1985} and note 126. \iput{Henkin principle}
\smallskip
...
note 28.
\medskip
\noindent{\bf FORM 44.} $DC(\aleph _{1})$:  Given a relation $R$ such that
...
\iput{Sikorski's extension theorem}
\smallskip
...
\medskip
\noindent{\bf FORM 51.} Cofinality Principle: Every linear ordering has a
cofinal sub well ordering.  \ac{Sierpi\'nski} \cite{1918} p 117.
\rightheadtext{FORM 51: Cofinality Principle}
\iput{cofinality principle}
\smallskip
...
\cite{1988a} and note 38.
\medskip
\noindent{\bf FORM 52.} Hahn-Banach Theorem:  If $V$ is a real vector space
...
\le p(x))$.  \ac{L\'os/Ryll-Nardzewski} \cite{1951}. \iput{Hahn-Banach
theorem}
\rightheadtext{FORM 52: Hahn Banach Theorem}
\smallskip
...
\cite{2001} \iput{Banach space} \iput{linear functional}
\medskip
\noindent{\bf FORM 53.} For all infinite cardinals $m$, $m^2\le
2^m$.  \ac{Mathias} \cite{1979} prob 1336.
\medskip
\noindent{\bf FORM 54.} For all infinite cardinals $m$, $m$ adj $2^{m}$
implies $m$ is an aleph.  ($m$ {\it adj} $n$ iff $m < n$ and $(\neg
\exists p) (m<p<n)$.)  \ac{Mathias} \cite{1979} prob 1337, thm 1338 and
prob 1339.
\medskip
\noindent{\bf FORM 55.}  For all infinite cardinals $m$ and $n$, if
$m\cdot n = m + n$ then, $m \le  n$ or $n \le m$. \ac{Mathias} \cite{1979}
p 125.
\medskip
\noindent{\bf FORM 56.} $\aleph(2^{\aleph_{0}})\neq\aleph_{\omega}$.
($\aleph(2^{\aleph_{0}})$ is Hartogs' aleph, the least $\aleph$
not $\le 2^{\aleph_{0}}$.)  \ac{Mathias} \cite{1979} p 125.
\medskip
\noindent{\bf FORM 57.} If $x$ and $y$ are Dedekind finite sets then either
$|x|\le |y|$ or $|y|\le |x|$.  \ac{Mathias} \cite{1979} p 125 and note 94.
\medskip
\noindent{\bf FORM 58.}  There is an ordinal $\alpha$ such that
...
\medskip
\noindent{\bf FORM 59($\le$).} If $(A,\le)$ is a partial ordering that
...
\medskip
\noindent{\bf FORM 60.} $C(\infty,WO)$: Every set of non-empty, well
...
\medskip
\noindent{\bf FORM 61.} $(\forall n\in\omega, n\ge 2$)$(C(\infty,n))$:
For each $n\in\omega$, $n\ge 2$, every set of $n$ element  sets
has a choice function.  \ac{Levy} \cite{1962}.
\medskip
\noindent{\bf FORM 62.}  $C(\infty,< \aleph_{0})$:  Every set of non-empty
finite  sets  has  a choice function.
\rightheadtext{FORM 62: $C(\infty,< \aleph_{0})$: Axiom of Choice for
Finite Sets}
\smallskip
...
\medskip
\noindent{\bf FORM 63.} $SPI$: Weak ultrafilter principle: Every infinite
set has a non-trivial ultrafilter. \ac{Jech} \cite{1973b} p 172 prob 8.5.
\medskip
\noindent{\bf FORM 64.} $E(I,Ia)$ (\ac{Howard/Yorke} \cite{1989}): There
...
compact.)  \ac{Brunner} \cite{1984b}.
\medskip
\noindent{\bf FORM 65.} The Krein-Milman Theorem: Let $K$ be a compact
...
\iput{Krein-Milman theorem}
\rightheadtext{FORM 65: Krein-Milman Theorem}
\medskip
\noindent{\bf FORM 66.} Every vector space over a field has a basis. \ac{Hamel}
\cite{1905}.
\medskip
\noindent{\bf FORM 67.} $MC(\infty,\infty)$ (MC), The Axiom of Multiple
Choice: For every set $M$ of non-empty sets there is a function $f$ such
that $(\forall x\in M)(\emptyset\neq f(x)\subseteq x$ and $f(x)$ is finite).
\rightheadtext{Form 67: Multiple Choice Axiom}
\smallskip
...
6.35 and 6.36}.
\medskip
\noindent{\bf FORM 68.} Nielsen-Schreier Theorem: Every subgroup of a
free group is free.  \ac{Jech} \cite{1973b} p 12.
\iput{Nielsen-Schreier theorem}
\rightheadtext{FORM 68: Nielsen-Schreier Theorem}
\medskip
\noindent{\bf FORM 69.}  Every field has an algebraic closure.  \ac{Jech}
\cite{1973b} p 13.
\medskip
\noindent{\bf FORM 70.} There is a non-trivial ultrafilter on $\omega$.
...
$C$ has an infinite subfamily with a choice function. \iput{restricted
choice} \ac{De la Cruz/Di Prisco} \cite{1998a}, \cite{1998b}.
\medskip
\noindent{\bf FORM 74.} For every $A\subseteq\Bbb R$ the following are
equivalent:
...
\medskip
\noindent{\bf FORM 75.} If a set has at least two elements, then it
can be partitioned into well orderable subsets, each of which has at
least two elements. \ac{Higasikawa} \cite{1995}.
\smallskip
\noindent{\bf FORM 76.} $MC_\omega(\infty,\infty)$ ($\omega$-MC): For
...
\ac{Howard/Keremedis/slash Rubin/ slash Stanley} \cite{1999} and note 141.
\medskip
\noindent{\bf FORM 77.} A linear ordering of a set $P$ is a well ordering
...
\ac{Sierpi\'nski} \cite{1918}.
\medskip
\noindent{\bf FORM 78.} Urysohn's Lemma:  If $A$ and $B$ are disjoint closed
sets in a normal space $S$, then there is a continuous $f:S\rightarrow
[0,1]$ which is 1 everywhere in $A$ and 0 everywhere in $B$.
\ac{Urysohn} \cite{1925} pp 290-292.  \iput{Urysohn's lemma}
\medskip
\noindent{\bf FORM 79.} ${\Bbb R}$ can be well ordered.  \ac{Hilbert}
\cite{1900} p 263.
\smallskip
...
\medskip
\noindent{\bf FORM 80.} $C(\aleph_{0},2)$:  Every denumerable set of  pairs
...
\medskip
\noindent{\bf FORM 81($n$).} (For $n\in\omega$) $K(n)$: For every
set $S$ there is an ordinal $\alpha$ and a one to one function
...
\medskip
\noindent{\bf FORM 82.} $E(I,III)$ (\ac{Howard/Yorke} \cite{1989}): If $X$
...
\medskip
\noindent{\bf FORM 86($\alpha$).} $C(\aleph_{\alpha},\infty)$:
...
$(P,\ge)$:
\itemitem{(a)} There is a cardinal $\mu< \lambda$ such that $(P,\ge)$ does
not contain a strong antichain of cardinality $\mu$.
\itemitem{(b)} $(P,\ge)$ contains a strong antichain of cardinality
$\lambda$.
\itemitem{(c)} $(P,\ge)$ contains a chain of cardinality $\lambda$.\par
...
note 29.
\medskip
\noindent{\bf FORM 87($\alpha$).}  $DC(\aleph_{\alpha})$: Given a relation
...
\medskip
\noindent{\bf FORM 88.}  $C(\infty ,2)$:  Every family of pairs has a
choice function.
\rightheadtext{Form 88:  $C(\infty ,2)$: Axiom of Choice for Pairs}
\smallskip
...
\medskip
\noindent{\bf FORM 89.} Antichain Principle:  Every partially ordered
set has a maximal antichain. \ac{Jech} \cite{1973b} p 133, \ac{Kurepa}
\cite{1953} and note 39.  \iput{antichain principle}
\smallskip
...
\cite{1976}.
\medskip
\noindent{\bf FORM 90.} $LW$:  Every linearly ordered set can be well
ordered. \ac{Jech} \cite{1973b} p 133 and \ac{Felgner/Jech} \cite{1973}.
\medskip
\noindent{\bf FORM 91.} $PW$:  The power set of a well ordered set can be
...
Note 75.
\medskip
\noindent{\bf FORM 92.} $C(WO,{\Bbb R})$:  Every well ordered family of
non-empty subsets of ${\Bbb R}$ has a choice function.
\ac{Howard/Keremedis/Rubin/Stanley/Tachtsis} \cite{1999}.
\medskip
\noindent{\bf FORM 93.} There is a non-measurable subset of ${\Bbb R}$.
\ac{Sierpi\'nski} \cite{1918}.
\medskip
\noindent{\bf FORM 94.} $C(\aleph_{0},\infty,{\Bbb R})$: Every denumerable
...
\itemitem{(1)} $(\forall\epsilon > 0)(\exists\delta >0)((\forall
y\in\Bbb R)(|y - x|<\delta\rightarrow |f(y) - f(x)|<\epsilon)$.
\itemitem{(2)} Whenever $\lim_{}x_{n} = x$, then
$\lim_{}f(x_{n})=f(x)$.
...
super second countable. \iput{super second countable} \ac{Gutierres}
\cite{2004} and note 159.
\smallskip
...
second countable.  \ac{Gutierres} \cite{2004} and  note 159. \iput{super
second countable}
\smallskip
...
\iput{super second countable}
\smallskip
\item{}{\bf [94 AE]} A second countable topological space is Hausdorff if
and only if every sequence has at most one limit.  \ac{Gutierres}
\cite{2004}.
\medskip
\noindent{\bf FORM 95($F$).} Existence of Complementary Subspaces over
...
p 148 prob 10.4.
\medskip
\noindent{\bf FORM 96.} L\"owig's Theorem. If $B_{1}$ and $B_{2}$ are
both bases for the vector space $V$ then $|B_{1}| = |B_{2}|$. \ac{L\"owig}
\cite{1934}. \iput{L\"owig's theorem}
\medskip
\noindent{\bf FORM 97.} Cardinal Representatives: For every set $A$ there
...
\iput{cardinal representatives}
\smallskip
\item{}{\bf [97 A]} For every set $\{X_i: i\in I\}$ of local cardinal
numbers, there is a set $\{x_i: i\in I\}$ of sets such that for each
...
\medskip
\noindent{\bf FORM 98.} The set of all finite subsets of a Dedekind finite
set is Dedekind finite. \ac{Jech} \cite{1973b} p 161 prob 11.5 and note 94.
\medskip
\noindent{\bf FORM 99.} Rado's Selection Lemma: Let $\{K(\lambda): \lambda
...
agree on S. \ac{Rado} \cite{1949}. \iput{Rado's selection lemma}
\smallskip
...
satisfying
\itemitem{(1)} $(\forall K\in T)(\exists S\in {\Cal F})(K\in S)$
\itemitem{(2)} $(\forall S_{1}, S_{2}\in{\Cal F})(S_{1}\cup S_{2}\in
{\Cal F})$
\item{}Assume there is a function $\phi$ with domain ${\Cal F}$ such that
for each $S\in{\Cal F}$, $\phi(S) = \phi_{S}$ is a function with domain
...
\medskip
\noindent{\bf FORM 101.}  Partition Principle:  If $S$ is a partition of
$M$, then $S \precsim M$.  \ac{Sierpi\'nski} \cite{1947}.
\iput{partition principle}
\rightheadtext{FORM 101: The Partition Principle}
\smallskip
\item{}{\bf [101 A]}  For all sets $x$ and $y$, $x \precsim^* y$ implies
$x\precsim y$.  \ac{Lindenbaum/Tarski} \cite{1926} and note 69.
\medskip
\noindent{\bf FORM 102.}  For all Dedekind finite cardinals $p$ and $q$, if
$p^{2} = q^{2}$ then $p = q$. \ac{Jech} \cite{1973b} p 162 prob 11.12 and
note 94.
\medskip
\noindent{\bf FORM 103.}  If $(P,<)$ is a linear ordering and
$|P| > \aleph_{1}$ then some initial segment of $P$ is uncountable.
\ac{Jech} \cite{1973b} p 164 prob 11.21.
\medskip
\noindent{\bf FORM 104.} There is a regular uncountable aleph. \ac{Jech}
\cite{1966b} p 165 prob 11.26.
\medskip
\noindent{\bf FORM 105.} There is a  partially ordered set $(A,\le)$ such
...
\medskip
\noindent{\bf FORM 106.} Baire Category Theorem for Compact Hausdorff
...
\medskip
\noindent{\bf FORM 107.}  M.~Hall's Theorem: Let $\{S(\alpha):
...
\iput{Hall's theorem}
\medskip
\noindent{\bf FORM 108.}  There is an ordinal $\alpha$ such that $2^{\aleph
_{\alpha}}$ is not the union of a denumerable set of denumerable sets.
\ac{Gitik} \cite{1985}.
\medskip
\noindent{\bf FORM 109.} Every field $F$ and every vector space
...
pp 119ff.
\medskip
\noindent{\bf FORM 110.} Every vector space over $\Bbb Q$ has a basis.
\ac{Blass} \cite{1984a}.
\medskip
\noindent{\bf FORM 111.} $UT(WO,2,WO)$: The union of an infinite
...
\medskip
\noindent{\bf FORM 112.} $MC(\infty,LO)$: For every family $X$ of non-empty
...
\medskip
\noindent{\bf FORM 113.} Tychonoff's Compactness Theorem for Countably Many
Spaces: The product of a countable set of compact spaces is compact.
\ac{Brunner} \cite{1985a}. \iput{Tychonoff's theorem}
\medskip
\noindent{\bf FORM 114.} Every A-bounded $T_2$ topological space is weakly
...
\medskip
\noindent{\bf FORM 115.} The product of weakly Loeb $T_2$ spaces is weakly
Loeb. (See form 114 for the definition of weakly Loeb.) \ac{Brunner}
\cite{1985a}.
\medskip
\noindent{\bf FORM 116.}  Every compact $T_2$ space is weakly  Loeb. (See
form 114 for the definition of weakly Loeb.)  \ac{Brunner} \cite{1985a}.
\medskip
\noindent{\bf FORM 117.}  If $\kappa$ is a measurable cardinal,
then $\kappa$ is the $\kappa$th inaccessible cardinal. \ac{Keisler/Tarski}
\cite{1964} and note 20.
\medskip
\noindent{\bf FORM 118.}  Every linearly orderable topological space is
normal.  \ac{Birkhoff} \cite{1967} p 241.
\rightheadtext{FORM 118: Linearly Ordered Spaces are Normal}
\smallskip
...
\cite{1997}.
\medskip
\noindent{\bf FORM 119.} $C(\aleph_{0}$,uniformly orderable with order type
...
\cite{1985}.
\iput{van Douwen's choice principle}
\medskip
\noindent{\bf FORM 120($K$).} If $K\subseteq\omega-\{0,1\}$, $C(LO,K)$:
Every linearly ordered set of non-empty sets each of whose cardinality
is in $K$ has a choice function.  \ac{Truss} \cite{1973a}.
\medskip
\noindent{\bf FORM 121.} $C(LO,<\aleph_{0})$: Every linearly ordered set of
non-empty finite sets has a choice function.  \ac{Truss} \cite{1973a}.
\medskip
\noindent{\bf FORM 122.} $C(WO,<\aleph_{0})$: Every well ordered set of
non-empty finite sets has a choice function.  \ac{Truss} \cite{1973a}.
\smallskip
...
\ac{Shannon} \cite{1988} p 569.)
\medskip
\noindent{\bf FORM 123.} $SPI^*$: Uniform weak ultrafilter principle: For
...
\medskip
\noindent{\bf FORM 124.} Every operator on a Hilbert space with an
...
\medskip
\noindent{\bf FORM 125.} There does not exist an infinite, compact connected
...
\medskip
\noindent{\bf FORM 126.} $MC(\aleph_0,\infty)$, Countable axiom of
...
\rightheadtext{Form 126: Countable Multiple Choice Axiom}
\smallskip
...
\medskip
\noindent{\bf FORM 127.} An amorphous power of a compact $T_2$ space, which
as a set is well orderable, is well orderable.  \ac{Brunner} \cite{1984b}.
\medskip
\noindent{\bf FORM 128.} Aczel's Realization Principle: On every infinite
set there is a Hausdorff topology with an infinite set of non-isolated
points.  \ac{Brunner} \cite{1984f}. \iput{Aczel's realization principle}
\medskip
\noindent{\bf FORM 129.} For every infinite set $A$, $A$ admits a partition
...
\cite{1984f}.
\medskip
\noindent{\bf FORM 130.} ${\Cal P}(\Bbb R)$ is well orderable. \ac{Brunner}
\cite{1984f}.
\medskip
\noindent{\bf FORM 131.} $MC_\omega(\aleph_0,\infty)$: For every
...
\medskip
\noindent{\bf FORM 133.} Every set is either well orderable or has an
infinite amorphous subset.  \ac{Brunner} \cite{1982a}.
\medskip
\noindent{\bf FORM 134.} If $X$ is an infinite $T_1$ space and $X^{Y}$
is $T_5$, then $Y$ is countable. ($T_5$ is ``hereditarily $T_4$''.)
\ac{A.~H.~Stone} \cite{1948}.
\medskip
\noindent{\bf FORM 135.} If $X$ is a $T_2$ space with at least two points
...
\medskip
\noindent{\bf FORM 136($k$).} Surjective Cardinal Cancellation (depends on
$k\in\omega-\{0\}$): For all cardinals $x$ and $y$, $kx\le^* ky$ implies
$x\le^* y$. \ac{Truss} \cite{1984} and note 69.
\medskip
\noindent{\bf FORM 137($k$).} Suppose $k\in\omega-\{0\}$. If $f$ is a 1-1
...
\medskip
\noindent{\bf FORM 138($k$).} Suppose $k\in\omega$.  If $f$ is a
...
\medskip
\noindent{\bf FORM 139.} Using the discrete topology on 2,
$2^{\Cal P(\omega)}$ is compact. \ac{Truss} \cite{1984}.
\medskip
\noindent{\bf FORM 140.} Let $\Omega$ be the set of all (undirected)
...
\medskip
\noindent{\bf FORM 141.} [14 P($n$)] with $n = 2$:  Let $\{A(i): i\in I\}$
...
\medskip
\noindent{\bf FORM 143.} $H(C,TR)$: If $(X,R)$ is a connected relation
...
and note 39.
\medskip
\noindent{\bf FORM 144.} Every set is almost well orderable. \ac{Keisler}
...
\ac{Ern\'e} \cite{2000} and note 154.  \iput{finite character}
\smallskip
...
note 94.
\medskip
\noindent{\bf FORM 146.}  $A(F,A1)$: For every $T_2$ topological space
...
\medskip
\noindent{\bf FORM 147.}  $A(D2)$:  Every $T_2$ topological space $(X,T)$
can be covered by a well ordered family of discrete sets. \ac{Brunner}
\cite{1983d} and note 26.
\medskip
\noindent{\bf FORM 148.} $A(W2,B1)$: For every $T_2$ topological space
$(X,T)$, if $X$ is well ordered, then $X$ has a well ordered base.
\ac{Brunner} \cite{1983d} and note 26.
\medskip
\noindent{\bf FORM 149.}  $A(F)$:  Every $T_2$ topological space is a
continuous, finite to one image of an A1 space. \ac{Brunner} \cite{1983d}
and note 26.
\medskip
\noindent{\bf FORM 150.} $PC(\infty,\aleph_0,\infty)$: Every infinite
set of denumerable sets has an infinite subset with a choice function.
\medskip
\noindent{\bf FORM 151.} $UT(WO,\aleph_{0},WO)$ ($U_{\aleph_{1}}$):
...
\medskip
\noindent{\bf FORM 152.} $D_{\aleph_{0}}$: Every non-well-orderable set is
...
\medskip
\noindent{\bf FORM 153.} The closed unit ball of a Hilbert space is compact
in the weak topology. \ac{Fossy/Morillon} \cite{1998} and
\ac{Rudin} \cite{1991}.
\medskip
\noindent{\bf FORM 154.} Tychonoff's Compactness Theorem for Countably
Many $T_2$ Spaces: The product of countably many $T_2$ compact spaces is
compact. \ac{Brunner} \cite{1983c}. \iput{Tychonoff's theorem}
\rightheadtext{Form: 154: A Countable Tychonoff Compactness Theorem}
\medskip
\noindent{\bf FORM 155.} $LC$: There are no non-trivial L\"auchli continua.
...
\medskip
\noindent{\bf FORM 156.} Theorem of Gelfand and Kolmogoroff: Two compact
$T_2$ spaces are  homeomorphic if their rings of real valued continuous
functions are isomorphic. \newline \ac{Brunner} \cite{1983a} and
\ac{Gelfand/Kolmogoroff} \cite{1939}. \iput{Gelfand-Kolmogoroff theorem}
\medskip
\noindent{\bf FORM 157.} Theorem of Goodner: A compact $T_{2}$ space is
...
\cite{1950}. \iput{Goodner's theorem}
\medskip
\noindent{\bf FORM 158.} In every Hilbert space $H$, if the closed unit
ball is sequentially compact, then $H$ has an orthonormal basis.
\ac{Brunner} \cite{1983b}.
\medskip
\noindent{\bf FORM 159.} The regular cardinals are cofinal in the class of
...
\medskip
\noindent{\bf FORM 160.} No Dedekind finite set can be mapped
onto an aleph. (See [9 E].)  \ac{Monro} \cite{1975} and note 94.
\medskip
\noindent{\bf FORM 161.}  Definability of cardinal addition in terms of
...
\medskip
\noindent{\bf FORM 162.} Non-existence of infinite units: There is no
infinite cardinal number $A$ such that $A + A > A$  and for all cardinals
$x$ and $y$, $x + y = A\rightarrow x = A$ or $y = A$. \ac{H\"aussler}
\cite{1983}.
\medskip
\noindent{\bf FORM 163.}  Every non-well-orderable set has an infinite,
Dedekind finite subset.  \ac{Brunner} \cite{1982a} and note 94.
\medskip
\noindent{\bf FORM 164.} Every non-well-orderable set has an infinite subset
with a Dedekind finite power set. \ac{Brunner} \cite{1982a}, notes 42
and 94.
\medskip
\noindent{\bf FORM 165.}  $C(WO,WO)$:  Every well ordered family of
non-empty, well orderable sets has a choice function.
\medskip
\noindent{\bf FORM 166.} $PC(\infty,2,\infty)$: Every infinite family of
pairs has an infinite subfamily with a choice function. \ac{Brunner}
\cite{1982a}.
\medskip
\noindent{\bf FORM 167.} $PKW(\aleph_{0},\ge 2,\infty)$, Partial
...
\medskip
\noindent{\bf FORM 168.}  Dual Cantor-Bernstein Theorem: $(\forall x)
(\forall y)(|x| \le^*|y|$ and $|y|\le^* |x|$ implies  $|x| = |y|)$ .
\ac{Banaschewski/Moore} \cite{1990} and note 69.
\iput{Cantor-Bernstein theorem}
\medskip
\noindent{\bf FORM 169.} There is an uncountable subset of ${\Bbb R}$
without a perfect subset. \ac{Banaschewski/Moore} \cite{1990},
\ac{Bernstein} \cite{1908}, and note 28.
\medskip
\noindent{\bf FORM 170.} $\aleph_{1}\le 2^{\aleph_{0}}$. \ac{Bernstein}
...
\medskip
\noindent{\bf FORM 171.} If $(P,\le)$ is a partial order such that $P$ is
...
\medskip
\noindent{\bf FORM 179($\epsilon$).} Suppose  $\epsilon > 0$ is an ordinal.
$\forall x$, $x\in W(\epsilon$). \ac{David} \cite{1980} and note 25.
\medskip
\noindent{\bf FORM 180.} Every Abelian group has a divisible
...
\rightheadtext{Form 180: Every Abelean group has a divisible hull.}
\medskip
\noindent{\bf FORM 181.} $C(2^{\aleph_0},\infty)$: Every set $X$ of
non-empty sets such that $|X|=2^{\aleph_0}$ has a choice function.
\ac{Freyd} \cite{1980}.
\medskip
\noindent{\bf FORM 182.} There is an aleph whose cofinality is greater
than $\aleph_{0}$.  \ac{Gitik} \cite{1980}.
\medskip
\noindent{\bf FORM 183($\alpha$).}  There are no $\aleph_{\alpha}$
...
\medskip
\noindent{\bf FORM 184.}  Existence of a double uniformization: For all
$X$ and $Y$, for all $E\subseteq X\times Y$, if there is an infinite
cardinal $\kappa$ satisfying:
\itemitem{(1)} $\forall y_0\in Y$, $|\{(x,y_0): x\in X\}\cap E| =\kappa$
and
\itemitem{(2)} $\forall x_0\in X$, $|\{(x_0,y): y\in Y\}\cap E| = \kappa$,
...
\ac{Kaniewski/Rogers} \cite{1980}.  \iput{double uniformization}
\medskip
\noindent{\bf FORM 185.}  Every linearly ordered Dedekind finite set is
...
\medskip
\noindent{\bf FORM 186.}  Every pair of cardinal numbers has a least upper
bound (in the usual cardinal ordering.)  \ac{Hickman} \cite{1979b}.
\medskip
\noindent{\bf FORM 187.}  Every pair of cardinal numbers has a greatest
lower bound (in the usual cardinal ordering.)  \ac{Hickman} \cite{1979b}.
\medskip
\noindent{\bf FORM 188.} $EP\ Ab$: For every Abelian group $A$ there is a
projective Abelian group $G$ and a homomorphism from $G$ onto $A$.
\ac{Blass}  \cite{1979}  and note 60.
\medskip
\noindent{\bf FORM 189.} $EI\ Ab$: For every Abelian group $A$ there is an
injective Abelian group $G$ and a one to one homomorphism from $A$ into $G$.
\ac{Blass} \cite{1979} and note 60.
\medskip
\noindent{\bf FORM 190.}  There is a non-trivial injective Abelian group.
\ac{Blass}  \cite{1979} and note 60.
\medskip
\noindent{\bf FORM 191.} $SVC$: There is a set $S$ such that for every set
$a$, there is an ordinal $\alpha$ and a function from $S\times\alpha$
onto $a$.  \ac{Blass} \cite{1979}.
\medskip
\noindent{\bf FORM 192.} $EP$ sets: For every set $A$ there is a projective
...
and 69.
\medskip
\noindent{\bf FORM 193.} $EFP\ Ab$: Every Abelian group is a homomorphic
image of a free projective Abelian group.  \ac{Blass} \cite{1979}
and note 60.
\medskip
\noindent{\bf FORM 194.} $C(\varPi^1_2)$ or $AC(\varPi^1_2)$: If $P\in
...
\medskip
\noindent{\bf FORM 195.}  Every general linear system has a linear global
reaction.  Note 62 and Harazisville (see \ac{Kharazishvili} \cite{1979}).
\medskip
\noindent{\bf FORM 196($\alpha$).} $\aleph_{\alpha}$ and
$\aleph_{\alpha+1}$ are not both measurable. \ac{Bull} \cite{1978} and
note 20.
\medskip
\noindent{\bf FORM 197.} ${\Bbb R}^{2}$ is the union of three sets $C$
with the property that for all $x\in C$ there is a straight line $L$
such that $L\cap C = \{x\}$.  \ac{Davies} \cite{1978}.
\medskip
\noindent{\bf FORM 198.} For every set $S$, if the only linearly orderable
...
\medskip
\noindent{\bf FORM 199($n$).} (For $n\in\omega-\{0,1\}$) If all
...
\medskip
\noindent{\bf FORM 208.} For all ordinals $\alpha$, $\aleph_{\alpha+1}\le
2^{\aleph_\alpha}$.  \ac{Figura} \cite{1977}.
\medskip
\noindent{\bf FORM 209.} There is an ordinal $\alpha$ such that for all
...
\cite{1970}.
\medskip
\noindent{\bf FORM 210.}  The commutator subgroup of a free group is free.
\ac{L\"auchli} \cite{1962}.
\medskip
\noindent{\bf FORM 211.} $DCR$: Dependent choice for relations on
...
\medskip
\noindent{\bf FORM 212.} $C(2^{\aleph_{0}},\subseteq{\Bbb R})$: If $R$ is a
...
\medskip
\noindent{\bf FORM 213.} $C(\infty,\aleph_{1})$: If $(\forall y\in X)(|y|
= \aleph_{1})$ then $X$ has a choice function.  \ac{Pincus} \cite{1977a}.
\medskip
\noindent{\bf FORM 214.} $Z(\omega)$: For every family $A$ of infinite
sets, there is a function $f$ such that for all $y\in A$, $f(y)$
is a non-empty subset of $y$ and $|f(y)|=\aleph_{0}$. \ac{Pincus}
\cite{1977a}.
\medskip
\noindent{\bf FORM 215.} If $(\forall y\subseteq X)(y$ can be linearly
ordered implies $y$ is finite), then $X$ is finite. \ac{Pincus}
\cite{1972a}.
\medskip
\noindent{\bf FORM 216.} Every infinite tree has either an infinite
chain or an infinite antichain.  \ac{Pincus} \cite{1972a} and note 21.
\medskip
\noindent{\bf FORM 217.} Every infinite partially ordered set has either
an infinite chain or an infinite antichain. \ac{Pincus} \cite{1972a}
and note 39.
\medskip
\noindent{\bf FORM 218.} $(\forall n\in\omega - \{0\}) MC(\infty,\infty
...
\medskip
\noindent{\bf FORM 219.}  $(\forall n\in\omega-\{0\}) MC(\infty,WO$,
...
\medskip
\noindent{\bf FORM 220($p$).} Suppose $p\in\omega$ and $p$ is a prime.
...
\cite{1977b}.
\medskip
\noindent{\bf FORM 221.} For all infinite $X$, there is a non-principal
measure on $\Cal P(X)$.  \ac{Pincus/Solovay} \cite{1977} and note 147.
\medskip
\noindent{\bf FORM 222.} There is a non-principal measure on
$\Cal P(\omega)$.  \ac{Pincus/Solovay} \cite{1977} and note 147.
\medskip
\noindent{\bf FORM 223.} There is an infinite set $X$ and a non-principal
measure on $\Cal P(X)$.  \ac{Pincus/Solovay} \cite{1977} and note 147.
\medskip
\noindent{\bf FORM 224.}  There is a partition of the real line into
...
\medskip
\noindent{\bf FORM 225.}  Every proper filter on $\omega$ can be extended
...
\medskip
\noindent{\bf FORM 226.}  Let $R$ be a commutative ring with identity,
...
\medskip
\noindent{\bf FORM 227.} For all groups $G$, if every finite subgroup of
$G$ can be fully ordered then $G$ can be fully ordered. \ac{Rav}
\cite{1977} and note 79.
\medskip
\noindent{\bf FORM 228.} Every torsion free Abelian group can be fully
ordered. \ac{Rav} \cite{1977} and note 79.
\medskip
\noindent{\bf FORM 229.} If $(G,\circ,\le)$ is a partially ordered group,
...
\medskip
\noindent{\bf FORM 230.} $L^{1} = HOD$. \ac{Szczepaniak} \cite{1977} and
note 82.
\medskip
\noindent{\bf FORM 231.} $UT(WO,WO,WO)$: The union of a well ordered
...
orderable.  Note 25.
\medskip
\noindent{\bf FORM 232.} Every metric space $(X,d)$ has a $\sigma$-point
...
\cite{1999} and note 141.
\medskip
\noindent{\bf FORM 233.} If a field has an algebraic closure it is unique
up to isomorphism. \ac{Mathias} \cite{1977b}. \iput{Artin-Schreier theorem}
\rightheadtext{Form 233: Fields Have At Most One Algebraic Closure}
\medskip
\noindent{\bf FORM 234.} There is a non-Ramsey set: There is a set $A$ of
...
\medskip
\noindent{\bf FORM 235.} If $V$ is a vector space and $B_{1}$ and $B_{2}$
are bases for $V$ then $|B_{1}|$ and $|B_{2}|$ are comparable. \ac{Gross}
\cite{1976}.
\medskip
\noindent{\bf FORM 236.} If $V$ is a vector space with a basis and $S$ is
...
\medskip
\noindent{\bf FORM 240.} If a group $G$ satisfies ``every ascending chain
of subgroups is finite," then every subgroup of $G$ is finitely generated.
\ac{Hickman} \cite{1976}.
\medskip
\noindent{\bf FORM 241.} Every algebraic closure of $\Bbb Q$ has a real
closed subfield.  \ac{Hodges} \cite{1976a}, \ac{L\"auchli} \cite{1962}
and note 84.
\medskip
\noindent{\bf FORM 242.} There is, up to an isomorphism, at most one algebraic
closure of ${\Bbb Q}$.  \ac{Hodges} \cite{1976a}, \ac{L\"auchli}
\cite{1962} and note 84.
\medskip
\noindent{\bf FORM 243.}  Every  principal ideal domain is a unique
factorization domain.  \ac{Hodges} \cite{1976a} and note 84.
\medskip
\noindent{\bf FORM 244.} Every principal ideal domain has a maximal ideal.
\ac{Hodges} \cite{1976a} and note 84.
\medskip
\noindent{\bf FORM 245.} There is a function $f :\omega_1\rightarrow
...
\medskip
\noindent{\bf FORM 246.} The monadic theory theory $MT(\omega_1,<)$ of
$\omega_1$ is recursive.  \ac{Litman} \cite{1976} and note 85.
\medskip
\noindent{\bf FORM 247.} Every atomless Boolean algebra is Dedekind
infinite.  \ac{Plotkin} \cite{1976}, notes 86 and 94.
\medskip
\noindent{\bf FORM 248.} For any $\kappa$, $\kappa$ is the cardinal number
of an infinite complete Boolean algebra if and only if $\kappa^{\aleph_0}
= \kappa $.  \ac{Plotkin} \cite{1976} and note 86.
\medskip
\noindent{\bf FORM 249.}  If $T$ is an infinite tree in
...
and note 39.
\medskip
\noindent{\bf FORM 260.} $Z(TR\&C,P)$: If $(X,R)$ is a transitive and
...
\medskip
\noindent{\bf FORM 261.} $Z(TR,T)$: Every transitive relation $(X,R)$ in
which every subset which is a tree has an upper bound, has a maximal
element. \ac{Harper/Rubin} \cite{1976} and note 39.
\medskip
\noindent{\bf FORM 262.} $Z(TR,R)$: Every transitive relation $(X,R)$ in
which every ramified subset $A$ has an upper bound, has a maximal element.
\ac{Harper/Rubin} \cite{1976} and note 39.
\medskip
\noindent{\bf FORM 263.} $H(AS\&C,P)$: Every every relation $(X,R)$ which is
antisymmetric and connected contains a $\subseteq$-maximal partially
ordered subset.  \ac{Harper/Rubin} \cite{1976} and note 39.
\medskip
\noindent{\bf FORM 264.} $H(C,P)$: Every connected relation $(X,R)$
contains a $\subseteq$-maximal partially ordered set. \ac{Harper/Rubin}
\cite{1976} and note 39.
\medskip
\noindent{\bf FORM 265.} $H(A,TR)$: Every relation $(X,R)$ contains a
$\subseteq$-maximal transitive subset.  \ac{Harper/Rubin} \cite{1976}
and note 39.
\medskip
\noindent{\bf FORM 266.} $H(AS,P)$: Every antisymmetric relation contains
$\subseteq$-maximal partially ordered subset. \ac{Harper/Rubin} \cite{1976}
and note 39.
\rightheadtext{FORMS 254-66: Maximal Principles}
\medskip
\noindent{\bf FORM 267.} There is no infinite, free complete Boolean
algebra.  \ac{Stavi} \cite{1975} and note 89.
\medskip
\noindent{\bf FORM 268.}   If ${\Cal L}$  is  a  lattice  isomorphic  to
...
\medskip
\noindent{\bf FORM 269.} For every cardinal $m$, there is a set $A$ such
that $2^{|A|^2}\ge m$ and there is a choice function on the collection
of 2-element subsets of $A$.  \ac{Lampe} \cite{1974}.
\medskip
\noindent{\bf FORM 270.} $CT_{\hbox{fin}}$: The compactness theorem for
...
\medskip
\noindent{\bf FORM 271($n$).} If $n\in\omega-\{0,1\}$, $CT_{n}$:
...
\ac{Kolany/Wojtylak} \cite{1991}.
\medskip
\noindent{\bf FORM 272.} There is an $X\subseteq{\Bbb R}$ such that
neither $X$ nor $\Bbb R - X$ has a perfect subset.
\ac{Truss} \cite{1974b} and note 28.
\medskip
\noindent{\bf FORM 273.} There is a subset of ${\Bbb R}$ which is not
Borel. \ac{Truss} \cite{1974b}.
\medskip
\noindent{\bf FORM 274.} There is a cardinal number $x$ and an $n\in\omega$
...
\cite{1974b}.
\medskip
\noindent{\bf FORM 275.} The sequence of cardinals $\langle\aleph_n: n
\in\omega\rangle$ has a unique minimal upper bound.  \ac{Truss}
\cite{1974b}.
\medskip
\noindent{\bf FORM 276.} $E(V'',III)$: For every set $A$, ${\Cal P}(A)$ is
Dedekind finite if and only if $A = \emptyset$  or $2|{\Cal P}(A)| >
|{\Cal P}(A)|$. \ac{Howard/Spi\u siak} \cite{1994} and note 94.
\medskip
\noindent{\bf FORM 277.} $E(D,VII)$: Every non-well-orderable cardinal is
decomposable. \ac{Monro} \cite{1973a} and note 94.
\medskip
\noindent{\bf FORM 278.} In an integral domain $R$, if every ideal is
finitely generated then $R$ has a maximal proper ideal. \ac{Hodges}
\cite{1974} and note 45 E.
\medskip
\noindent{\bf FORM 279.} The Closed Graph Theorem for operations
...
\cite{1963}.
\medskip
\noindent{\bf FORM 286.} Extended Krein-Milman Theorem:  Let K be a
...
\iput{Krein-Milman theorem}
\medskip
\noindent{\bf FORM 287.} The Hahn-Banach Theorem for Separable
...
\iput{Hahn-Banach theorem}
\medskip
\noindent{\bf FORM 288($n$).} If $n\in\omega-\{0,1\}$, $C(\aleph_0,n)$:
Every denumerable set of $n$-element sets has a choice function.
\ac{Wi\'sniewski} \cite{1972}.
\medskip
\noindent{\bf FORM 289.} If $S$ is a set of subsets of a countable set
and $S$ is closed under chain unions, then $S$ has a $\subseteq$-maximal
element.  \ac{Smullyan} \cite{1994} and note 102.
\medskip
\noindent{\bf FORM 290.}  For all infinite $x$, $|2^x|=|x^x|$. \ac{Pincus}
\cite{1978} and note 64.
\rightheadtext{Form 290: For all infinite $x$, $|2^x|=|x^x|$}
\medskip
\noindent{\bf FORM 291.}  For all infinite $x$, $|x!|=|x^x|$. \ac{Pincus}
\cite{1978} and note 64.
\medskip
\noindent{\bf FORM 292.} $MC(LO,\infty)$: For each linearly ordered family
...
\medskip
\noindent{\bf FORM 293.}  For all sets $x$ and $y$, if $x$ can be
linearly ordered and there is a mapping of $x$ onto $y$, then
$y$ can be linearly ordered.
\medskip
\noindent{\bf FORM 294.}  Every linearly ordered $W$-set is well
orderable.  \ac{Morris} \cite{1969} and note 25.
\medskip
\noindent{\bf FORM 295.}  DO:  Every infinite set has a dense linear
...
\medskip
\noindent{\bf FORM 296.} Part-$\infty$: Every infinite set is the disjoint
union of infinitely many infinite sets.  \ac{Gonzalez} \cite{1995b},
\ac{Pincus} \cite{1997}.
\medskip
\noindent{\bf FORM 297.} Extremally disconnected compact Hausdorff spaces
...
\medskip
\noindent{\bf FORM 298.} Every compact Hausdorff space has a Gleason cover.
\ac{Bell} \cite{1988}, \ac{Gleason} \cite{1958}, and note 114.
\medskip
\noindent{\bf FORM 299.} Any extremally disconnected compact Hausdorff
space is projective in the category of Boolean topological spaces.
\ac{Bell} \cite{1988}, notes 60 and 114.
\medskip
\noindent{\bf FORM 300.} Any continuous surjection between extremally
...
\medskip
\noindent{\bf FORM 306.} The set of \icopy{Vitali equivalence classes} is
...
\medskip
\noindent{\bf FORM 307.} If $m$ is the cardinality of the set of
...
\cite{1991}.
\medskip
\noindent{\bf FORM 308($p$).} If $p$ is a prime and if $\{G_y: y\in Y\}$
...
\cite{1987}.
\medskip
\noindent{\bf FORM 309.} The Banach-Tarski Paradox: There are three finite
...
\cite{1924}.
\iput{Banach-Tarski paradox}
\rightheadtext{Form 309: The Banach-Tarski Paradox}
\medskip
\noindent{\bf FORM 310.} The Measure Extension Theorem: Suppose that $\Cal
...
\iput{measure extension theorem}
\medskip
\noindent{\bf FORM 311.} Abelian groups are amenable. ($G$ is {\it
...
\medskip
\noindent{\bf FORM 312.} A subgroup of an amenable group is amenable.
...
\medskip
\noindent{\bf FORM 313.} $\Bbb Z$ (the set of integers under addition) is
...
\medskip
\noindent{\bf FORM 314.} For every set $X$ and every permutation $\pi$
...
\cite{2000}.
\medskip
\noindent{\bf FORM 315.} $\Omega = \omega_1$, where  $\Omega =
...
\ac{Gitik} \cite{1980}.
\medskip
\noindent{\bf FORM 316.} If a linearly ordered set $(A,\le)$ has the fixed
...
\medskip
\noindent{\bf FORM 317.} Weak Sikorski Theorem:  If $B$ is a complete,
...
\cite{1973}.
\medskip
\noindent{\bf FORM 318.} $\aleph_1$ is not measurable.
\ac{Jech} \cite{1968a} and note 20.
\medskip
\noindent{\bf FORM 319.} Measurable cardinals are inaccessible. \ac{Drake}
\cite{1974} and note 20.
\medskip
\noindent{\bf FORM 320.} No successor cardinal, $\aleph_{\alpha+1}$, is
measurable. \ac{Drake} \cite{1974} and note 20.
\medskip
\noindent{\bf FORM 321.} There does not exist an ordinal
$\alpha$ such that $\aleph_{\alpha}$ is weakly compact and
$\aleph_{\alpha+1}$ is measurable. \ac{Bull} \cite{1978} and note 20.
\medskip
\noindent{\bf FORM 322.} $KW(WO,\infty)$, The Kinna-Wagner Selection
...
\medskip
\noindent{\bf FORM 323.} $KW(\infty,WO)$, The Kinna-Wagner Selection
...
\medskip
\noindent{\bf FORM 324.} $KW(WO,WO)$, The Kinna-Wagner Selection Principle
...
\medskip
\noindent{\bf FORM 325.} Ramsey's Theorem II: $\forall n,m\in\omega$, if
...
\iput{Ramsey's theorem}
\medskip
\noindent{\bf FORM 326.}  2-SAT:  Restricted Compactness Theorem for
...
\medskip
\noindent{\bf FORM 327.} $KW(WO,<\aleph_0)$,  The Kinna-Wagner Selection
...
(See form 15.) \ac{Howard/A.~Rubin/J\.~Rubin} \cite{1997}.
\medskip
\noindent{\bf FORM 328.} $MC(WO,\infty)$: For  every well ordered set
...
\cite{1997}.
\medskip
\noindent{\bf FORM 329.} $MC(\infty,WO)$: For  every set $M$ of well
...
\medskip
\noindent{\bf FORM 330.} $MC(WO,WO)$: For every well ordered set $X$ of
...
\medskip
\noindent{\bf FORM 331.} If $(X_i)_{i\in I}$ is a family of compact
...
\medskip
\noindent{\bf FORM 332.}  A product of non-empty compact sober
topological spaces is non-empty.  \ac{Morillon} \cite{1988} and note 71.
\medskip
\noindent{\bf FORM 333.} $MC(\infty,\infty,\hbox{ odd})$: For every
...
\cite{1996b}.
\medskip
\noindent{\bf FORM 334.} $MC(\infty,\infty,\hbox{ even})$: For every
...
\cite{1996a}.
\medskip
\noindent{\bf FORM 335($n$).} Every quotient group of an Abelian group
each of whose elements has order  $\le n$ has a set of representatives.
\ac{Keremedis} \cite{1996b}.
\medskip
\noindent{\bf FORM 336($n$).} (For $n\in\omega$, $n\ge 2$.)  For every
...
\medskip
\noindent{\bf FORM 337.} $C(WO,\hbox{uniformly linearly ordered})$:  If
...
\ac{Howard/Keremedis/Rubin/Stanley/Tachtsis} \cite{1999}.
\medskip
\noindent{\bf FORM 338.}  $UT(\aleph_0,\aleph_0,WO)$: The union
of a denumerable number of denumerable sets is well orderable.
Note 4.
\medskip
\noindent{\bf FORM 339.}  Martin's Axiom $(\aleph_{0})$: Whenever
...
\iput{Martin's axiom}
\medskip
\noindent{\bf FORM 340.} Every Lindel\"of metric space is separable.
\ac{Good/Tree} \cite{1995}.
\medskip
\noindent{\bf FORM 341.} Every Lindel\"of metric space is second countable.
\ac{Good/Tree} \cite{1995}.
\rightheadtext{FORMS 340 and 341: Properties of Lindel\"of Metric Spaces}
\medskip
\noindent{\bf FORM 342($n$).}  (For $n\in\omega$, $n\ge 2$.)
...
\medskip
\noindent{\bf FORM 343.}  A product of non-empty, compact $T_2$
topological spaces is non-empty.  \ac{Morillon} \cite{1988}.
\medskip
\noindent{\bf FORM 344.}  If $(E_i)_{i\in I}$ is a family of non-empty
sets, then there is a family $(U_i)_{i\in I}$ such that $\forall i\in I$,
$U_i$ is an ultrafilter on $E_i$.  \ac{Morillon} \cite{1988}.
\medskip
\noindent{\bf FORM 345.}  Rasiowa-Sikorski Axiom:  If $(B,\land,\lor)$
...
\icopy{Rasiowa-Sikorski property}.  \ac{Morillon} \cite{1988} and note 71.
\smallskip
...
\medskip
\noindent{\bf FORM 351.} A countable product of metrizable spaces
is metrizable. \ac{Keremedis} \cite{1996b} and note 132.
\medskip
\noindent{\bf FORM 352.} A countable product of second countable
spaces is second countable. \ac{Keremedis} \cite{1996b} and note 132.
\medskip
\noindent{\bf FORM 353.} A countable product of first countable spaces
is first countable. \ac{Keremedis} \cite{1996b} and note 132.
\medskip
\noindent{\bf FORM 354.} A countable product of separable $T_2$ spaces
is separable. \ac{Keremedis} \cite{1996b} and note 132.
\medskip
\noindent{\bf FORM 355.} $KW(\aleph_0,\infty)$, The Kinna-Wagner Selection
...
then $\emptyset\neq f(A)\subsetneq A$.
\medskip
\noindent{\bf FORM 356.} $KW(\infty,\aleph_0)$, The Kinna-Wagner Selection
...
if $|A| > 1$  then $\emptyset\neq f(A)\subsetneq A$.
\medskip
\noindent{\bf FORM 357.} $KW(\aleph_0,\aleph_0)$, The Kinna-Wagner
...
\medskip
\noindent{\bf FORM 358.} $KW(\aleph_0,<\aleph_0)$, The Kinna-Wagner
...
\medskip
\noindent{\bf FORM 359.} If $\{A_{x}: x\in S\}$ and $\{B_{x}:
x\in S\}$ are families  of pairwise disjoint sets and $ |A_{x}|
\le |B_{x}|$ for all $x\in S$, then $|\bigcup_{x\in S}A_{x}|
\le |\bigcup_{x\in S} B_{x}|$. \ac{Higasikawa} \cite{1995}.
\medskip
\noindent {\bf FORM 360.} A system of linear equations over the field
$\{0,1\}$ has a solution, if and only if every finite subsystem has
a solution. \ac{Brunner} \cite{1999}.
\medskip
\noindent{\bf FORM 361.} In $\Bbb R$, the union of a denumerable
number of analytic sets is analytic.  G\. \ac{Moore} \cite{1982},
pp 181 and 325.
\medskip
\noindent{\bf FORM 362.} In $\Bbb R$, every Borel set is analytic.
G\. \ac{Moore} \cite{1982}, pp 181 and 325.
\medskip
\noindent{\bf FORM 363.} There are exactly $2^{\aleph_0}$ Borel sets
in $\Bbb R$.  G\. \ac{Moore} \cite{1982}, p 325.
\medskip
\noindent{\bf FORM 364.} In $\Bbb R$, there is a measurable set
that is not Borel.  G\. \ac{Moore} \cite{1982}, p 325.
\medskip
\noindent{\bf FORM 365.}  For every uncountable set $A$, if $A$ has the
same cardinality as each of its uncountable subsets then
$|A| = \aleph_1$.  \ac{Sierpi\'nski} \cite{1918}.
\medskip
\noindent{\bf FORM 366.}  There is a discontinuous function $f: \Bbb R
\to\Bbb R$ such that for all real $x$ and $y$, $f(x+y)=f(x)+f(y)$.
\ac{Hamel} \cite{1905}.
\medskip
\noindent{\bf FORM 367.} There is a Hamel basis for $\Bbb R$ as a
vector space over $\Bbb Q$.  \ac{Hamel} \cite{1905}.
\medskip
\noindent{\bf FORM 368.} The set of all denumerable subsets of
$\Bbb R$ has power $2^{\aleph_0}$.  \ac{Sierpi\'nski} \cite{1917}.
\medskip
\noindent{\bf FORM 369.} If $\Bbb R$ is partitioned into two sets, at
least one of them has cardinality $2^{\aleph_0}$. \ac{Luzin/Sierpi\'nski}
\cite{1917}.
\medskip
\noindent{\bf FORM 370.} Weak Gelfand Extreme Point Theorem: If $A$ is
...
29.  \iput{Gelfand extreme point theorem}
\smallskip
...
and note 147.
\medskip
\noindent{\bf FORM 371.}  There is an infinite, compact, Hausdorff,
extremally disconnected topological space.  \ac{Morillon} \cite{1993}
and note 114.
\medskip
\noindent{\bf FORM 372.} Generalized Hahn-Banach Theorem:  Assume that
...
\ac{Schechter} \cite{1996b} and note 31. \iput{Hahn-Banach theorem}
\rightheadtext{Form 372: Generalized Hahn-Banach Theorem}
\smallskip
...
\ac{Schechter} \cite{1996b} and note 31. \iput{convex domination theorem}
\smallskip
...
and note 31. \iput{support theorem}
\smallskip
...
and note 31. \iput{sandwich theorem}
\medskip
\noindent{\bf FORM 373($n$).} (For $n\in\omega$, $n\ge 2$.)
...
\medskip
\noindent{\bf FORM 374($n$).} $UT(\aleph_0,n,\aleph_0)$ for $n\in\omega
...
\medskip
\noindent{\bf FORM 375.} Tietze-Urysohn Extension Theorem: If
...
\iput{Tietze-Urysohn theorem}
\medskip
\noindent{\bf FORM 376.} Restricted Kinna Wagner Principle:  For every
...
\cite{1998a}.
\medskip
\noindent{\bf FORM 377.} Restricted Ordering Principle:  For every
infinite set $X$ there is an infinite subset $Y$ of $X$ such that $Y$
can be linearly ordered.  De la Cruz/Di \ac{Prisco} \cite{1998a}.
\medskip
\noindent{\bf FORM 378.} Restricted Choice for Families of Well Ordered
...
has a choice function.  De la Cruz/Di \ac{Prisco} \cite{1998a}.
\medskip
\noindent{\bf FORM 379.} $PKW(\infty,\infty,\infty)$: For every infinite
...
\medskip
\noindent{\bf FORM 380.} $PC(\infty,WO,\infty)$:  For every infinite
...
\medskip
\noindent{\bf FORM 381.} DUM:  The disjoint union of metrizable spaces
is metrizable. \ac{Howard\slash Keremedis/slash Rubin/slash Rubin}
\cite{1998b}.
\medskip
\noindent{\bf FORM 382.} DUMN:  The disjoint union of metrizable spaces
is normal. \ac{Howard\slash Keremedis/slash Rubin/slash Rubin} \cite{1998b}.
\medskip
%NOTE that 383 and 232 are equivalent. Therefore, 383, and [383 A]-
%[383 C] have become [232 H]-[232 K].
\medskip
\noindent{\bf FORM 384.} Closed Filter Extendability for $T_1$ Spaces.
...
\medskip
\noindent{\bf FORM 385.} Countable Ultrafilter Theorem:  Every proper
filter with a countable base over a set $S$ (in ${\Cal P}(S)$) can
be extended to an ultrafilter. \ac{Herrlich/Keremedis} \cite{1999a}.
\medskip
\noindent{\bf FORM 386.} Every B compact (pseudo)metric space is Baire.
\ac{Herrlich/Keremedis} \cite{1999a}, notes 6, 10, and 28.
\medskip
\noindent{\bf FORM 387.} DPO:  Every infinite set has a non-trivial,
...
\cite{1995a}.
\medskip
\noindent{\bf FORM 388.} Every infinite branching poset (a partially
...
\cite{1999a}.
\medskip
\noindent{\bf FORM 389.} $C(\aleph_0,2,\Cal P({\Bbb R}))$: Every
denumerable family of two element subsets of $\Cal P({\Bbb R})$ has
a choice function.  \ac{Keremedis} \cite{1999b}.
\medskip
\noindent{\bf FORM 390.} Every infinite set can be partitioned either
...
\cite{1989}.
\medskip
\noindent{\bf FORM 391.} $C(\infty,LO)$: Every set of non-empty
linearly orderable sets has a choice function.
\medskip
\noindent{\bf FORM 392.} $C(LO,LO)$: Every linearly ordered set of
linearly orderable sets has a choice function.
\medskip
\noindent{\bf FORM 393.} $C(LO,WO)$: Every linearly ordered set of
non-empty well orderable sets has a choice function.
\medskip
\noindent{\bf FORM 394.} $C(WO,LO)$: Every well ordered set of
non-empty linearly orderable sets has a choice function.
\medskip
\noindent{\bf FORM 395.} $MC(LO,LO)$: For each linearly ordered family
...
\medskip
\noindent{\bf FORM 396.} $MC(LO,WO)$: For each linearly ordered family
of non-empty well orderable sets $X$, there is a function $f$ such
that for all $x\in X$ $f(x)$ is a non-empty, finite subset of $x$.
\medskip
\noindent{\bf FORM 397.} $MC(WO,LO)$: For each well ordered family
of non-empty linearly orderable sets $X$, there is a function $f$ such
that for all $x\in X$ $f(x)$ is a non-empty, finite subset of $x$.
\medskip
\noindent{\bf FORM 398.} $KW(LO,\infty)$, The Kinna-Wagner Selection
...
if $|A|>1$ then $\emptyset\neq f(A)\subsetneq A$.
\medskip
\noindent{\bf FORM 399.} $KW(\infty,LO)$, The Kinna-Wagner Selection
...
$A\in M$, if $|A|>1$ then $\emptyset\neq f(A)\subsetneq A$.
\medskip
\noindent{\bf FORM 400.} $KW(LO,LO)$, The Kinna-Wagner Selection
...
$\emptyset\neq f(A)\subsetneq A$.
\medskip
\noindent{\bf FORM 401.} $KW(LO,<\aleph_0)$, The Kinna-Wagner Selection
...
topology. \ac{Delhomm\'e/Morillon} \cite{2000} and note 23.
\medskip
\noindent{\bf FORM 413.} Every infinite set $S$ is the union of a
set, well-ordered by inclusion, of subsets which are
non-equipollent to $S$.  \ac{Ern\'e} \cite{2000} and note 154.
\medskip
\noindent{\bf FORM 414.}  Every $\Cal W$-frame is a $\Cal
D$-frame.  \ac{Ern\'e} \cite{2000} and note 154.
\medskip
\noindent{\bf FORM 415.}  Every $\Cal W$-compactly generated
complete lattice is algebraic.  \ac{Ern\'e} \cite{2000} and note 154.
\medskip
\noindent{\bf FORM 416.} Every non-compact topological space $S$ is
...
\medskip
\noindent{\bf FORM 417.}  On every non-trivial Banach space there is a
non-trivial linear functional (bounded or unbounded).  \ac{Luxemburg/V\"ath}
\cite{2001} \iput{Banach space} \iput{linear functional}
\medskip
\noindent{\bf FORM 418.}  DUM($\aleph_0$): The countable disjoint union
of metrizable spaces is metrizable. \ac{De la Cruz/Hall/Howard/
Keremedis/Rubin} \cite{2002a}.
\medskip
\noindent{\bf FORM 419.} UT($\aleph_0$,cuf,cuf): The union of a denumerable
set of cuf sets is cuf. (A set is {\it cuf} if it is a countable union of
finite sets.) \ac{De la Cruz/Hall/Howard/Keremedis/Rubin} \cite{2002a}.
\smallskip
...
\medskip
\noindent{\bf FORM 420.}  UT($\aleph_0$,$\aleph_0$,cuf): The union of a
denumerable set of denumerable sets is cuf. \ac{De la Cruz/Hall/Howard/
Keremedis/Rubin} \cite{2002a}.
\medskip
\noindent{\bf FORM 421.}  $UT(\aleph_0,WO,WO)$: The union of a
denumerable set of well orderable sets can be well ordered.
\medskip
\noindent{\bf FORM 422($n$).} $UT(WO,n,WO)$, $n\in \omega-\{0,1\}$: The
...
function.
\medskip
\noindent{\bf FORM 423.} $\forall n\in \omega-\{o,1\}$, $C(\aleph_0, n)$ :
...
\medskip
\noindent{\bf FORM 424.} Every Lindel\"of metric space is super second
countable.  \ac{Gutierres} \cite{2004} and note 159. \iput{super second
countable}
\medskip
\noindent{\bf FORM 425.} For every first countable topological space $(X,
\Cal T)$ there is a family $(\Cal D(x))_{x \in X}$ such that $\forall x
\in X$, $D(x)$ countable local base at $x$.  \ac{Gutierres} \cite{2004}
and note 159.
\medskip
\noindent{\bf FORM 426.}  If $(X,\Cal T) $ is a first countable
...
($MC_\omega(\infty,\infty)$) and form 32 ($C(\aleph_0,\aleph_0)$).
\ac{Gutierres} \cite{2004}.
\medskip
\noindent{\bf FORM 427.} $\exists F$ AL20($F$): There is a field $F$ such
that every vector space $V$ over $F$ has the property that every
independent subset of $V$ can be extended to a basis.  \ac{Bleicher}
\cite{1964}, \ac{Rubin, H.\/Rubin, J.} \cite{1985, p.119, AL20}.
\medskip
\noindent{\bf FORM 428.} $\exists F$ B$(F)$:  There is a field $F$ such
that every vector space over $F$ has a basis. \ac{Bleicher} \cite{1964},
\ac{Rubin, H.\/Rubin, J.} \cite{1985, p.119, B}.
\medskip
\noindent{\bf FORM 429($p$).} (Where $p$ is a prime) B: Every vector space
over $\mathbb Z_p$ has a basis.  ($\mathbb Z_p$ is the $p$ element field.)
\ac{Bleicher} \cite{1964}, \ac{Rubin, H.\/Rubin, J.} \cite{1985, p.119,
B}.
\medskip
\noindent{\bf FORM 430($p$).} (Where $p$ is a prime) AL21$(p)$: Every
vector space over $\mathbb Z_p$ has the property that for every subspace
$S$ of $V$, there is a subspace $S'$ of $V$ such that $S \cap S' = \{ 0
\}$ and $S \cup S'$ generates $V$ in other words such that $V = S \oplus
S'$.  \ac{Bleicher} \cite{1964}, \ac{Rubin, H.\/Rubin, J.} \cite{1985,
p.119, AL21}.
\smallskip
\item{}{\bf [430 A($p$)].} (Where $p$ is a prime) MC$(\infty,\infty,\text{
relatively prime to p})$, MC4$(p)$:  For every set $X$ of non-empty sets
there is a function $f$ with domain $X$ such that $\forall u \in X$,
$f(u)$ is a finite non-empty subset of $u$ such that $|f(u)|$ and $p$ are
relatively prime. See form 218 and \ac{Bleicher} \cite{1964}, \ac{Rubin,
H.\/Rubin, J.} \cite{1985, p.124, theorems 6.37 and 6.38}.
\medskip
